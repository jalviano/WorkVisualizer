...
plot_work <- function(dom_scores) {
    # pdf(file='../data/guided_work.pdf', pointsize=20, family='serif', width=8, height=5)
...
plot_trial <- function(dom_scores) {
    # pdf(file='../data/guided_work.pdf', pointsize=20, family='serif', width=8, height=5)
    t <- theme(axis.title=element_text(size=20),
               legend.text=element_text(size=20),
               legend.title=element_text(''),
...
work <- function(subject, test_sig_map, utility, mutants, test_trials) {
    ...
    for (n in 1:test_trials) {
        ...
        results <- format_results(dom_scores)
    }
    dom_scores <- format_results(dom_scores)
    plot_work(dom_scores)
}

trial <- function(subject, test_sig_map, utility, mutants, test_trials) {
    total_dom_nodes <- length(unique(get_test_sigs(subject, test_sig_map)))
    live <- mutants[order(utility, decreasing=TRUE)]
    scores <- list()
    test_sigs <- list()
    ...
    scores <- as.numeric(unlist(scores))
    results <- data.frame(work=1:length(scores), score=scores)
    return(results)
}
